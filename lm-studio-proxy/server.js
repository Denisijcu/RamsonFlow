...
const SUPPORTED_MODELS = [
  ...
  { id: 'gemma3:4b',           name: 'Gemma 3 4B (Ollama)',     size: '3.3GB', speed: 'fast',   recommended: false },
...
function toNativeFormat(messages, model, temperature, maxTokens) {
  ...
  const parts = userMsgs.map(m => (m.role === 'user' ? 'User: ' : 'Assistant: ') + m.content);
  const input = userMsgs.length === 1 ? userMsgs[0].content : parts.join('\n');
...
async function callLmStudio(messages, model, temperature, maxTokens) {
  ...
  if (settings.apiMode === 'native') {
    const resp = await axios.post(`${settings.lmStudioUrl}/api/v1/chat`, toNativeFormat(messages, model, temperature, maxTokens), { timeout: 60000 });
    return toOpenAIFormat(resp.data, model);
  }
  const resp = await axios.post(`${settings.lmStudioUrl}/v1/chat/completions`, { model, messages, temperature, max_tokens: maxTokens }, { timeout: 60000 });
  return resp.data;
}
...
app.post('/settings', (req, res) => {
  ...
  console.log(`[LM Proxy] Settings → model: ${settings.activeModel} | mode: ${settings.apiMode}`);
...
app.get('/health', async (req, res) => {
  try {
    ...
    await axios.get(`${settings.lmStudioUrl}${endpoint}`, { timeout: 3000 });
...
app.post('/v1/chat/completions', async (req, res) => {
  ...
  try {
    ...
    res.json({ id: 'mock-' + Date.now(), object: 'chat.completion', model, choices: [{ index: 0, message: { role: 'assistant', content: `[Mock] Server unreachable at ${settings.lmStudioUrl}.` }, finish_reason: 'stop' }], usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } });
  }
});
